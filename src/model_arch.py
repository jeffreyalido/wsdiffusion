...
RECIP_ROOT_2 = 1.0 / math.sqrt(2.0)
...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, t: torch.Tensor):
        ...
        t = t * 1000  # for the proper scales for positional encoding (jalido)
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        * `t` has shape `[batch_size, time_channels]`
        ...
        return (h + self.shortcut(x)) * RECIP_ROOT_2
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: Optional[torch.Tensor] = None):
        ...
        * `t` has shape `[batch_size, time_channels]`
        ...
        # attn = attn.softmax(dim=2) # default. problematic with nans in the gradient
        attn = torch.softmax(
            10 * torch.tanh(attn), dim=2
        )  # https://discuss.pytorch.org/t/runtimeerror-function-softmaxbackward0-returned-nan-values-in-its-0th-output/171776/3
        # Multiply by values
...
class MiddleBlock(nn.Module):
    ...
    def __init__(self, n_channels: int, time_channels: int, dropout: float = 0.1):
        super().__init__()
        self.res1 = ResidualBlock(
            n_channels, n_channels, time_channels, dropout=dropout
        )
...
class Upsample(nn.Module):
    ...
    def __init__(self, n_channels):
        ...
        self.conv = nn.Upsample(scale_factor=2, mode="nearest", align_corners=None)
        self.conv2 = nn.Conv2d(
            n_channels, n_channels, padding="same", kernel_size=(3, 3)
        )
        # self.swish = Swish()
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        n_blocks: int = 4,  # originally 2 for ddpm
        dropout: float = 0.1,
    ):
        """
    ...
    def _init_weights(self, m):
        """
        Initialize the weights of Conv2d modules with He initialization.
        """
